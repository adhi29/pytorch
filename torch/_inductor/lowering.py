...
def _local_scalar_dense(data):
    ...
    if unbacked_bindings:
        ...
        if isinstance(expr, sympy.Symbol) and symbol_is_type(
            expr, (SymT.UNBACKED_INT, SymT.UNBACKED_FLOAT)
        ):
            binding_sym = expr
        else:
            raise AssertionError(
                "local_scalar_dense expected unbacked_bindings or an existing "
                f"unbacked symbol in node metadata, got {val!r}"
            )
        keypath = ()
